...
const openNotebookButtonRef = ref<InstanceType<typeof HeaderButton> | null>(
  null,
);
const createSimpleNotebookRef = ref<InstanceType<typeof HeaderButton> | null>(
  null,
);
...
// Saves the current notebook on server
    ...
    try {
      ...
      if (props.currentNotebook._id && props.currentNotebook._id !== "") {
        response = await axios.post(
          `/api/notebooks/${authStore.user?.email}/${props.currentNotebook._id}`,
          props.currentNotebook,
        );
      } else {
        response = await axios.post(
          `/api/notebooks/${authStore.user?.email}`,
          newNotebook,
        );
...
onMounted(async () => {
  ...
  classes.value = allClasses.filter((cls: any) =>
    userClassesId.includes(cls._id),
  );
});
</script>
  ...
  <div>
    ...
    >
      <div class="flex-1 flex items-center justify-start gap-2">
        ...
        />
        <h1
          class="text-white text-[1.563rem] font-semibold font-['Inter'] px-2"
        >
          Easy Notes
        </h1>
      </div>
      ...
      <div class="flex-1 flex items-center justify-end gap-2">
        ...
        >
          <div
            class="flex flex-col items-center justify-center p-4 min-w-[8rem]"
          >
            ...
            <div
              ...
              class="flex flex-col items-center gap-2 animate-fade-in"
            >
              <div
        ...
        >
          ...
          >
            ...
            />
            <div class="relative font-medium shrink-0">
              {{ props.username }}
            </div>
          </div>
        </RouterLink>
