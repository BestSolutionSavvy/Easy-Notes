...
const handleDelete = async (lectureId: string) => {
  ...
  try {
    ...
    const updatedPdfs = (props.classItem.pdfs || []).filter(id => id !== lectureId);
  ...
  <div
    ...
    <div
      ...
      <div class="self-stretch flex items-center gap-[0.625rem] text-[1.875rem] text-darkslategray">
        <div class="relative font-semibold">{{ props.classItem?.name || 'Select a class' }}</div>
      ...
      <ListElement v-for="(lecture, index) in lectures" :key="lecture._id" :title="lecture.name"
        :date="formatDate(lecture.date)" :index="index" :buttons="props.role === 'teacher'
          ? [{ icon: trashIcon, alt: 'Delete', onClick: () => handleDelete(lecture._id) }]
          : [{ icon: loadIcon, alt: 'Load', onClick: () => handleLoad(lecture._id) }]" />
    </div>
  </div>
</template>