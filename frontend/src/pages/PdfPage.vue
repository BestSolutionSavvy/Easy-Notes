...
const currentHighlights = computed(
  () => currentPageData.value?.highlights || [],
);

const pdfUrl = computed(() =>
  pdfFile.value?.data ? URL.createObjectURL(pdfFile.value.data) : null,
);

const pdfWidth = computed(() =>
  basePageWidth.value > 0
    ? (basePageWidth.value * scale.value) / 100
    : undefined,
);

const pdfHeight = computed(() =>
  basePageHeight.value > 0
    ? (basePageHeight.value * scale.value) / 100
    : undefined,
);
                ...
                if (currentPageData.value) {
                  ...
                  for (const existingHighlight of highlights) {
                    if (!intesrects) {
                      ...
                      const newHighlightArea =
                        newHighlight.width * newHighlight.height;
...
watch(
  () => props.notebook?.id_pdf,
  async (newPdfId, oldPdfId) => {
  ...
  >
    ...
    >
      ...
      <div class="overflow-hidden flex items-start p-[0.625rem] gap-[0.937rem]">
        ...
        <div
          @click="pdfTemplateRef?.download(pdfFile?.name || 'download.pdf')"
